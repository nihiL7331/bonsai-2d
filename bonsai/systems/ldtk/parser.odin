...
loadData :: proc(name: WorldName) -> bool {
	if name == WorldName.nil {
...
calculateWorldPosition :: proc() {
	...
	for &level in _world.levels {
		if _world.worldLayout == WorldLayout.Free || _world.worldLayout == WorldLayout.GridVania {
		...
		if _world.worldLayout == WorldLayout.LinearHorizontal do worldPosition += gmath.Vec2Int{level.pxWidth, 0}
		else if _world.worldLayout == WorldLayout.LinearVertical do worldPosition -= gmath.Vec2Int{0, level.pxHeight}
	}
}

calculateWorldEntityPosition :: proc(level: Level) {
...
generateWorldColliders :: proc() {
	for &level in _world.levels {
		...
		found := false
		for layer in layers {
			if layer.type == "IntGrid" && layer.identifier == COLLISIONS_LAYER_IDENTIFIER {
				grid = layer.intGrid
		...
		for y := 0; y < gridHeight; y += 1 {
			...
			for x := 0; x < gridWidth; x += 1 {
				...
				isWall := (value == INTGRID_WALL_VALUE)
...
generateEntityCustomFields :: proc() {
	for level in _world.levels {
		if layers, ok := level.layerInstances.?; ok {
			for layer in layers {
				for &entity in layer.entityInstances {
					entity.customFields = make(map[string]FieldInstanceType)
