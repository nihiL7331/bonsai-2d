...
getFont :: proc(fontName: game.FontName, size: uint) -> (Font, bool) {
	if fontName == .nil {
	...
	filename := game.fontFilename[fontName]
	...
	bitmap := make([]u8, _BITMAP_WIDTH * _BITMAP_HEIGHT)
	defer delete(bitmap)
...
//
// **Does not handle wrapping.** Useful for centering text.
getTextSize :: proc(fontName: game.FontName, fontSize: uint, text: string) -> gmath.Vector2 {
